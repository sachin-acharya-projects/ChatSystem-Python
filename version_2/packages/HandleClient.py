...
class HandleClient:
    ...
    def connectToserver(self):
        ...
        while True:
            try:
                ...
                if not isInformed:
                    ...
                    self.window.addstr(
                        "[SERVER OFFLINE] WAITING FOR CONNECTION")
        ...
        self.username_header: bytes = f"{len(self.username):<{self.HEADER_LENGTH}}".encode(
            'utf-8')
        self.client_socket.send(self.username_header + self.username)

    def sendMessage(self, message: str):
        message: bytes = message.encode('utf-8')
        message_header: bytes = f"{len(message):<{self.HEADER_LENGTH}}".encode(
            'utf-8')
        try:
    ...
    def getMessage(self):
        try:
            username_header: bytes = self.client_socket.recv(
                self.HEADER_LENGTH)
            ...
            username: str = self.client_socket.recv(
            ...
            message: str = self.client_socket.recv(
